...
compact_literal (kissat * solver, unsigned dst_lit, unsigned src_lit)
{
  ...
  if (GET_OPTION(ccanr)) {
    solver->ls_active[dst_idx] = solver->ls_active[src_idx];
    solver->ls_best_soln[dst_idx] = solver->ls_best_soln[src_idx];
    solver->ls_mediation_soln[dst_idx] = solver->ls_mediation_soln[src_idx];
    solver->phase_value[dst_idx] = solver->phase_value[src_idx];
    solver->top_trail_soln[dst_idx] = solver->top_trail_soln[src_idx];
  }
